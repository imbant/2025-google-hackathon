...
const GiftDetail: React.FC = () => {
  ...
  const generateGiftImage = async (giftData: PresentRecommend) => {
    ...
    try {
      ...
      // 构造专业的图片生成 prompt
      const contents = `请创建一个精美的产品展示图片，展示以下礼物：${
        giftData.presentName
      }。
...
，请确保图片风格与推荐理由和总结相符。`;
  ...
  if (!gift) {
    return (
      <div style={{ textAlign: "center", marginTop: 80 }}>未找到礼物详情</div>
    );
  }
    ...
    <Layout style={{ minHeight: "100vh", padding: 24 }}>
      <Card style={{ maxWidth: 600, margin: "40px auto", borderRadius: 18 }}>
        ...
        <div style={{ marginBottom: 24, textAlign: "center" }}>
          <div style={{ fontWeight: 600, color: "#CA555D", marginBottom: 12 }}>
            礼物预览
          </div>
          {imageLoading ? (
            <div
        ...
        >
          ...
          {gift.presentSuggestion && (
            <>
              <div
                style={{ fontWeight: 600, color: "#CA555D", marginBottom: 12 }}
              >
                送礼建议
              </div>
              <div style={{ color: "#444", marginBottom: 18 }}>
          ...
          <div style={{ color: "#888", marginBottom: 18 }}>
            {gift.tags && gift.tags.length > 0 ? gift.tags.join(" / ") : "无"}
          </div>
          <div style={{ fontWeight: 600, color: "#CA555D", marginBottom: 12 }}>
            价格区间
          </div>
          <div style={{ color: "#888" }}>
            {typeof gift.minPrice === "number" &&
            typeof gift.maxPrice === "number"
              ? `${gift.minPrice}元 - ${gift.maxPrice}元`
              : "未知"}
          </div>
        </div>
